...
describe('SurveyList', () => {
  ...
  it('Should logout on on logout link click', () => {
    mockUnexpectedError()
    cy.visit('')
  ...
  it('Should present survey items', () => {
    ...
    cy.get('li:nth-child(1)').then(li => {
      ...
      cy.fixture('icons').then(icon => assert.equal(li.find('[data-testid="icon"]').attr('src'), icon.thumbUp))
    })
    cy.get('li:nth-child(2)').then(li => {
      ...
      cy.fixture('icons').then(icon => assert.equal(li.find('[data-testid="icon"]').attr('src'), icon.thumbDown))
    })
  })
})
